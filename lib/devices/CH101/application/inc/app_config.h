 ...
 *
 *	 The CHIRP_FIRST_SENSOR_MODE value specifies the mode for the first
 *sensor (lowest numbered) that is present.  If only one sensor is attached,
 *this value must be either CH_MODE_TRIGGERED_TX_RX or CH_MODE_FREERUN.
 *
 *	 The CHIRP_OTHER_SENSOR_MODE value specifies the mode for all other
 ...
 */
#define CHIRP_SENSOR_STATIC_RANGE       \
  (0) /* static target rejection sample \
